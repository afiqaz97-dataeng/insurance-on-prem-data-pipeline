...
8. **GitHub Actions `services:` containers can't override `command:`** — use `bitnami/minio` (auto-starts from env vars), not plain `minio/minio` (requires an explicit `server /data` command that can't be injected via the `services:` block).
