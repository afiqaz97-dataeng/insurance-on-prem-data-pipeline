...
- [x] Built and saved for real in Power BI Desktop, walked through live: connected to `CuratedTakafulPOC`, relationships set up (caught and removed an auto-detected `dim_policies`↔`dim_participants` relationship — exactly the fan-out risk flagged in the guide), all 7 dashboard tiles built. Fund-split and PIF-leakage checks both rendered as empty tables (pass); freshness cards showed real timestamps matching the actual pipeline run; fund-segregation-by-category chart visually confirmed `General` products carry zero `pif_amount` while `Family` products don't — the core business rule, now visible, not just tested
